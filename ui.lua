function starport_scene()
  ...
  gs['player'] = {x=76,y=64,sprite=2}
  gs['customs'] = {x=43,y=64,sprite=7}
  gs['trader'] = {x=23,y=24,sprite=8}
  gs['fueler'] = {x=100,y=64,sprite=10}
  gs['a_prompt'] = {x=100,y=100,sprite=9}
  slf.wandering = interface()
  slf.wandering._current_splat='player'
  slf.wandering.actors = {'player','customs','trader','fueler'}
  slf.wandering._splats = {
  ...
  slf.wandering.draw = function(me)
  ...
  slf.wandering.update = function(me)
    --Handle Motion
    local x = gs['player'].x
    local y = gs['player'].y
    local dx = 0
    ...
    for k in all(me.actors) do
      if k != 'player' then
        if dsto(gs[me._splats[k].ref], gs['player']) < 8 then
          target_npc = k
        end
      end
    end
  ...
  -- A talk dialog 
  slf.talking = interface()
  slf.talking._current_splat='pay_customs'
  slf.talking._splats = {
  ...
  slf.talking.draw = function(me)
    me._splats.text_area:draw()
    me._splats.pay_customs:draw()
  end
  slf.talking.update = function(me)
    local dir = nil
